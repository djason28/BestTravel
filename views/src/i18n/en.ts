export const en = {
  home_title: "Discover Unforgettable Travel Experiences",
  home_subtitle: "Handpicked packages across Bintan & Batam",
  explore_packages: "Explore Packages",
  contact_us: "Contact Us",
  about_us: "About Us",
  quick_links: "Quick Links",
  packages: "Packages",
  search_placeholder: "Search packages...",
  no_packages: "No packages found",
  clear_filters: "Clear Filters",
  featured: "Featured",
  duration: "Duration",
  destination: "Destination",
  starting_from: "Starting from",
  starting_price_sgd: "Starts at just",
  starting_price_tagline: "Sweet deal, no stress. Lock your ride now.",
  details: "Details",
  language_toggle_en: "English",
  language_toggle_zh: "中文",
  packages_page_title: "Explore Our Packages",
  packages_page_subtitle:
    "Discover amazing destinations and unforgettable experiences",
  filters: "Filters",
  active: "Active",
  categories: "Categories",
  match: "Match",
  any: "Any",
  all: "All",
  availability: "Availability",
  sort_by: "Sort By",
  newest_first: "Newest First",
  most_popular: "Most Popular",
  price_low_high: "Price: Low to High",
  price_high_low: "Price: High to Low",
  duration_short_long: "Duration: Short to Long",
  duration_long_short: "Duration: Long to Short",
  inquiries_most: "Most Inquiries",
  inquiries_least: "Least Inquiries",
  featured_only: "Featured only",
  not_featured: "Not featured",
  price_range: "Price Range",
  min: "Min",
  max: "Max",
  duration_days: "Duration (days)",
  participants: "Participants",
  clear_all_filters: "Clear All Filters",
  loading: "Loading...",
  packages_found_suffix: "packages found",
  showing: "Showing",
  of: "of",
  page: "Page",
  pages: "Pages",
  packages_label: "packages",
  previous: "Previous",
  next: "Next",
  contact_page_subtitle: "Get in touch with our travel experts",
  send_us_message: "Send Us a Message",
  full_name: "Full Name",
  email_address: "Email Address",
  phone_number: "Phone Number",
  subject: "Subject",
  message: "Message",
  send_message: "Send Message",
  contact_information: "Contact Information",
  address: "Address",
  phone: "Phone",
  email: "Email",
  business_hours: "Business Hours",
  quick_contact: "Quick Contact",
  quick_contact_desc:
    "Need immediate assistance? Chat with us on WhatsApp for instant support!",
  chat_on_whatsapp: "Chat on WhatsApp",
  visit_our_office: "Visit Our Office",
  feature_safe_secure_title: "Safe & Secure",
  feature_safe_secure_desc:
    "Travel with confidence knowing your safety is our priority",
  feature_best_prices_title: "Best Prices",
  feature_best_prices_desc:
    "Competitive pricing with no hidden fees or surprises",
  feature_expert_guides_title: "Expert Guides",
  feature_expert_guides_desc:
    "Professional local guides with extensive knowledge",
  feature_support_title: "24/7 Support",
  feature_support_desc: "Round-the-clock assistance whenever you need it",
  footer_tagline: "Your trusted travel partner for unforgettable adventures.",
  popular_destinations: "Popular Destinations",
  privacy_policy: "Privacy Policy",
  terms_of_service: "Terms of Service",
  faq: "FAQ",
  copyright_prefix: "©",
  all_rights_reserved: "All rights reserved.",
  home: "Home",
  contact_whatsapp: "Contact via WhatsApp",
  error_title: "Something went wrong",
  error_message:
    "We apologize for the inconvenience. Please try refreshing the page.",
  refresh_page: "Refresh Page",
  go_home: "Go Home",
  dest_bali: "Bali",
  dest_lombok: "Lombok",
  dest_yogyakarta: "Yogyakarta",
  dest_raja_ampat: "Raja Ampat",
  back_to_packages: "Back to Packages",
  package_not_found: "Package not found",
  browse_all_packages: "Browse all packages",
  group_size: "Group Size",
  views: "Views",
  overview: "Overview",
  highlights: "Highlights",
  itinerary: "Itinerary",
  day_label: "Day",
  whats_included: "What's Included",
  whats_excluded: "What's Excluded",
  per_person: "per person",
  book_via_whatsapp_cta: "Book via WhatsApp",
  need_help: "Need Help?",
  need_help_desc:
    "Have questions or want a custom trip? Contact our travel team!",
  header_subtagline: "Trusted Indonesian Travel Experts",
  // About Page
  about_title: "About Best Travel Agency",
  about_subtitle: "Creating unforgettable travel experiences since 2015",
  our_story: "Our Story",
  story_p1:
    "Founded in 2015, Best Travel Agency was born from a passion for exploration and a desire to share the beauty of Indonesia with travelers from around the world. What started as a small team of dedicated travel enthusiasts has grown into one of the most trusted travel agencies in the region.",
  story_p2:
    "We believe that travel is more than just visiting new places—it's about creating meaningful connections, experiencing different cultures, and making memories that last a lifetime. Our mission is to craft personalized travel experiences that go beyond the ordinary, taking you to hidden gems and iconic destinations alike.",
  story_p3:
    "With over a decade of experience and thousands of satisfied travelers, we continue to innovate and improve our services, ensuring that every journey with us is exceptional. Our deep knowledge of Indonesia's diverse landscapes, rich culture, and warm hospitality allows us to create authentic experiences that truly capture the spirit of each destination.",
  story_text:
    "Founded in 2015, Best Travel Agency was born from a passion for exploration and a desire to share the beauty of Indonesia with travelers from around the world. What started as a small team of dedicated travel enthusiasts has grown into one of the most trusted travel agencies in the region.\n\nWe believe that travel is more than just visiting new places—it's about creating meaningful connections, experiencing different cultures, and making memories that last a lifetime. Our mission is to craft personalized travel experiences that go beyond the ordinary, taking you to hidden gems and iconic destinations alike.\n\nWith over a decade of experience and thousands of satisfied travelers, we continue to innovate and improve our services, ensuring that every journey with us is exceptional. Our deep knowledge of Indonesia's diverse landscapes, rich culture, and warm hospitality allows us to create authentic experiences that truly capture the spirit of each destination.",
  why_choose_us: "Why Choose Us",
  safe_secure: "Safe & Secure",
  safe_secure_desc:
    "Your safety is our top priority. We partner with certified operators and maintain the highest safety standards in all our tours.",
  award_winning: "Trusted by Travelers",
  award_winning_desc:
    "Thousands of happy travelers trust us for reliable service, seamless planning, and unforgettable experiences across Indonesia.",
  personalized_exp: "Personalized Experiences",
  personalized_exp_desc:
    "Every traveler is unique. We customize each itinerary to match your interests, preferences, and travel style.",
  expert_guides: "Expert Local Guides",
  expert_guides_desc:
    "Our professional guides are passionate locals who share their deep knowledge and love for their homeland with every guest.",
  sustainable_tourism: "Sustainable Tourism",
  sustainable_tourism_desc:
    "We are committed to responsible travel practices that benefit local communities and preserve natural environments.",
  best_value: "Best Value",
  best_value_desc:
    "Competitive pricing without compromising quality. We offer transparent pricing with no hidden fees or surprises.",
  our_values: "Our Values",
  excellence: "Excellence",
  excellence_desc:
    "We strive for perfection in every detail, ensuring each journey exceeds expectations.",
  passion: "Passion",
  passion_desc:
    "Our love for travel and Indonesia drives us to create truly exceptional experiences.",
  integrity: "Integrity",
  integrity_desc:
    "We operate with honesty, transparency, and respect for our clients and partners.",
  by_the_numbers: "By the Numbers",
  years_experience: "Years of Experience",
  happy_travelers: "Happy Travelers",
  destinations: "Destinations",
  satisfaction_rate: "Satisfaction Rate",
  our_commitment: "Our Commitment",
  commit_1: "Providing authentic, culturally immersive experiences",
  commit_2: "Supporting local communities and sustainable tourism",
  commit_3: "Maintaining the highest standards of safety and service",
  commit_4: "Offering transparent pricing with no hidden costs",
  commit_5: "Being available 24/7 for our travelers",
  commit_6: "Continuously improving and innovating our offerings",
  cta_title: "Ready to Start Your Adventure?",
  cta_subtitle:
    "Join thousands of satisfied travelers and experience Indonesia like never before",
  story_combined:
    "Founded in 2015, Best Travel Agency was born from a passion for exploration and a desire to share the beauty of Indonesia with travelers from around the world. What started as a small team of dedicated travel enthusiasts has grown into one of the most trusted travel agencies in the region.\n\nWe believe that travel is more than just visiting new places—it's about creating meaningful connections, experiencing different cultures, and making memories that last a lifetime. Our mission is to craft personalized travel experiences that go beyond the ordinary, taking you to hidden gems and iconic destinations alike.\n\nWith over a decade of experience and thousands of satisfied travelers, we continue to innovate and improve our services, ensuring that every journey with us is exceptional. Our deep knowledge of Indonesia's diverse landscapes, rich culture, and warm hospitality allows us to create authentic experiences that truly capture the spirit of each destination.",
  header_subtagline_header: "Exclusive Travel",
  // Cars Page
  cars_page_title: "Car Rental",
  cars_page_subtitle:
    "Explore our fleet of quality rental vehicles for your journey",
  no_cars: "No cars found",
  search_cars_placeholder: "Search cars...",
  filter_all_transmission: "All Transmissions",
  filter_driver_all: "With / Without Driver",
  filter_self_drive: "Self Drive",
  try_change_filter: "Try adjusting your search filters",
  services: "Services",
  car_rental_service: "Car Rental",
  // Car Detail Page
  back_to_cars: "Back to Cars",
  car_not_found: "Car not found",
  browse_all_cars: "Browse all cars",
  car_rental_detail: "Car Rental Details",
  seats: "Seats",
  fuel_type: "Fuel Type",
  price_unit: "Price Unit",
  min_days: "Min. Days",
  with_driver_label: "With Driver",
  without_driver_label: "Self Drive",
  transmission_label: "Transmission",
  car_features: "Car Features",
  car_details: "Car Details",
  book_car_whatsapp_cta: "Book via WhatsApp",
  car_rental_inquiry: "Car Rental Inquiry",
  driver_option: "Driver Option",
  per_day: "per day",
  per_trip: "per trip",
  per_hour: "per hour",
  automatic_transmission: "Automatic",
  manual_transmission: "Manual",
};
