...
export function useLocalizedPackage<T extends Record<string, any>>(pkg: T | null) {
  ...
  if (Array.isArray(pkg.itinerary)) {
    mapped.itinerary = pkg.itinerary.map((it: any) => {
      ...
      if (Array.isArray(it.activitiesZh) && it.activitiesZh.length > 0) copy.activities = it.activitiesZh;
      if (Array.isArray(it.mealsZh) && it.mealsZh.length > 0) copy.meals = it.mealsZh;
