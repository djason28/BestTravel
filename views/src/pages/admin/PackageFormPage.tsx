...
import type { PackageFormData, PackageImage, PricePair } from "../../types";
...
export const PackageFormPage: React.FC = () => {
  ...
  const onChange = (
    ...
    setForm((prev) => ({
      ...prev,
      [name]: ["duration", "minParticipants", "maxParticipants"].includes(name)
        ? Number(value)
        : value,
    }));
  };
  ...
  const onPriceChange = (
    ...
    setForm((prev) => {
      ...
      return {
        ...
        currency: prices[0]?.currency ?? "IDR",
      };
    });
  };
  ...
  const addPrice = () => {
    setForm((prev) => ({
      ...prev,
      prices: [...prev.prices, { amount: 0, currency: "IDR" }],
    }));
  };

  const removePrice = (idx: number) => {
    setForm((prev) => {
      const prices = prev.prices.filter((_, i) => i !== idx);
      return {
        ...prev,
        prices,
        price: prices[0]?.amount ?? 0,
  ...
  const onSubmit = async () => {
    if (!form.prices || form.prices.length === 0) {
      addToast("At least one price is required", "error");
      return;
    }
    if (form.prices[0].amount < 1) {
      addToast("Price must be at least 1", "error");
      return;
    }
    if (form.minParticipants < 1) {
      addToast("Min participants must be at least 1", "error");
      return;
    }
    ...
    if (form.minParticipants > form.maxParticipants) {
      addToast("Min participants cannot exceed Max participants", "error");
      return;
    }
    if (!form.title.trim()) {
    ...
    try {
      const payload: PackageFormData = {
        ...
        excluded: excludedText
          ...
          .map((s) => s.trim())
          .filter(Boolean),
        includedZh: includedZhText
          .split("\n")
          .map((s) => s.trim())
          .filter(Boolean),
        excludedZh: excludedZhText
          .split("\n")
          .map((s) => s.trim())
          .filter(Boolean),
      };
      (payload as any).images = images.map((img, idx) => ({
        url: img.url,
      ...
      addToast(
        e instanceof Error ? e.message : "Failed to save package",
        "error",
      );
    } finally {
      setSaving(false);
  ...
  const progressPercent =
    step === 1 ? (englishRequiredOk ? 50 : 25) : mandarinRequiredOk ? 100 : 75;
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          ...
          <p className="text-gray-600 mt-1">
            {step === 1
              ? "Step 1: English Content & Details"
              : "Step 2: Mandarin Content & Publish"}
          </p>
          <div className="flex gap-2 mt-2 text-xs">
            <span
              className={`px-2 py-1 rounded ${englishRequiredOk ? "bg-green-100 text-green-700" : "bg-yellow-100 text-yellow-700"}`}
            >
              EN {englishRequiredOk ? "Complete" : "Incomplete"}
            </span>
            <span
              className={`px-2 py-1 rounded ${mandarinRequiredOk ? "bg-green-100 text-green-700" : "bg-yellow-100 text-yellow-700"}`}
            >
              ZH {mandarinRequiredOk ? "Complete" : "Incomplete"}
            </span>
          </div>
        </div>
        <div className="flex items-center gap-2 text-sm font-medium">
          <span
            className={`px-3 py-1 rounded-full ${step === 1 ? "bg-blue-600 text-white" : "bg-gray-200 text-gray-700"}`}
          >
            1. English
          </span>
          <span
            className={`px-3 py-1 rounded-full ${step === 2 ? "bg-blue-600 text-white" : "bg-gray-200 text-gray-700"}`}
          >
            2. Mandarin
          </span>
        </div>
          ...
          <div className="p-6 space-y-6">
            <div className="border-b pb-4">
              ...
              <p className="text-sm text-gray-600 mt-1">
                Essential details (English version)
              </p>
            </div>
            <Input
              label="Title"
              ...
              value={form.title}
              onChange={onChange}
              placeholder="Bali Paradise Tour"
              required
            />
            <Input
              label="Availability"
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  Categories (e.g., beach, foodie, historical)
                </label>
                <div className="space-y-2">
                  <input
                    ...
                    onKeyDown={(e) => {
                      if (["Enter", ",", ";"].includes(e.key)) {
                        e.preventDefault();
                        const tokens = tokenizeCategories(categoryInput);
                        if (tokens.length) {
                          setForm((prev) => ({
                            ...prev,
                            categories: Array.from(
                              new Set([...prev.categories, ...tokens]),
                            ),
                          }));
                          setCategoryInput("");
                        }
                  ...
                  <input
                    ...
                    onKeyDown={(e) => {
                      if (["Enter", ",", ";"].includes(e.key)) {
                        e.preventDefault();
                        const tokens = tokenizeCategories(highlightInput);
                        if (tokens.length) {
                          setForm((prev) => ({
                            ...prev,
                            highlights: Array.from(
                              new Set([...(prev.highlights || []), ...tokens]),
                            ),
                          }));
                          setHighlightInput("");
                        }
                    ...
                    className="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                    placeholder="Scenic beach, Local cuisine"
                  />
            ...
            <div className="border-b pb-4 mt-4">
              ...
              <p className="text-sm text-gray-600 mt-1">
                Set pricing and capacity details
              </p>
            </div>

            {/* Dynamic price-currency list */}
            <div className="space-y-2">
              <div className="grid grid-cols-12 gap-3 text-xs font-semibold text-gray-500 uppercase tracking-wide px-1">
                <span className="col-span-7">Harga</span>
                <span className="col-span-4">Mata Uang</span>
                <span className="col-span-1" />
              </div>
              {form.prices.map((p: PricePair, idx: number) => (
                <div key={idx} className="grid grid-cols-12 gap-3 items-center">
                  <input
                    ...
                    className="col-span-7 px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm"
                  />
                  <select
                    value={p.currency}
                    onChange={(e) =>
                      onPriceChange(idx, "currency", e.target.value)
                    }
                    className="col-span-4 px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm"
                  >
              ...
              <button
                type="button"
                onClick={addPrice}
                className="flex items-center gap-2 text-sm text-blue-600 hover:text-blue-700 font-medium mt-1 px-1"
              >
              ...
              {form.prices.length > 1 && (
                <p className="text-xs text-gray-500 px-1">
                  Harga pertama akan ditampilkan kepada pengunjung sebagai harga
                  utama.
                </p>
              )}
            </div>
            ...
            <div className="border-b pb-4 mt-8">
              ...
              <p className="text-sm text-gray-600 mt-1">
                Upload beautiful images of your package
              </p>
            </div>
                    ...
                    <div key={idx} className="relative group aspect-square">
                      ...
                      <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity rounded-lg flex flex-col items-center justify-center gap-2">
                        {!img.isCover && (
                          <button
                            type="button"
                            className="px-3 py-1.5 bg-yellow-400 hover:bg-yellow-500 text-gray-900 rounded-md text-sm font-medium"
                            onClick={() =>
                              setImages((prev) =>
                                prev.map((im, i) => ({
                                  ...im,
                        ...
                        <button
                          type="button"
                          className="px-3 py-1.5 bg-red-500 hover:bg-red-600 text-white rounded-md text-sm font-medium"
                          onClick={() =>
                            setImages((prev) =>
            ...
            <div className="flex justify-between pt-4">
              ...
              <Button
                onClick={() => {
                  if (!englishRequiredOk) {
                    addToast("Fill required English fields", "error");
                    return;
                  }
                  setStep(2);
                }}
        ...
        <Card>
          <div className="p-6 space-y-6">
            <div className="border-b pb-4">
              ...
              <p className="text-sm text-gray-600 mt-1">填写该套餐的中文内容</p>
            </div>
            <Input
              label="标题 (Chinese Title)"
              name="titleZh"
              value={form.titleZh || ""}
              onChange={onChange}
              placeholder="巴厘岛天堂之旅"
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div>
                ...
                <div className="space-y-2">
                  <input
                    type="text"
                    value={categoryZhInput}
                    onChange={(e) => setCategoryZhInput(e.target.value)}
                    onKeyDown={(e) => {
                      if (["Enter", ",", ";"].includes(e.key)) {
                        e.preventDefault();
                        const tokens = tokenizeCategories(categoryZhInput);
                        if (tokens.length) {
                          setForm((prev) => ({
                            ...prev,
                            categoriesZh: Array.from(
                              new Set([
                                ...(prev.categoriesZh || []),
                                ...tokens,
                              ]),
                    ...
                    onBlur={() => {
                      ...
                      if (tokens.length) {
                        setForm((prev) => ({
                          ...prev,
                          categoriesZh: Array.from(
                            new Set([...(prev.categoriesZh || []), ...tokens]),
                          ),
                        }));
                        setCategoryZhInput("");
                      }
                  ...
                  <input
                    type="text"
                    value={highlightZhInput}
                    onChange={(e) => setHighlightZhInput(e.target.value)}
                    onKeyDown={(e) => {
                      if (["Enter", ",", ";"].includes(e.key)) {
                        e.preventDefault();
                        const tokens = tokenizeCategories(highlightZhInput);
                        if (tokens.length) {
                          setForm((prev) => ({
                            ...prev,
                            highlightsZh: Array.from(
                              new Set([
                                ...(prev.highlightsZh || []),
                                ...tokens,
                              ]),
                    ...
                    onBlur={() => {
                      ...
                      if (tokens.length) {
                        setForm((prev) => ({
                          ...prev,
                          highlightsZh: Array.from(
                            new Set([...(prev.highlightsZh || []), ...tokens]),
                          ),
                        }));
                        setHighlightZhInput("");
                      }
              ...
              <Input
                label="目的地 (Destination Zh)"
                name="destinationZh"
                value={form.destinationZh || ""}
                onChange={onChange}
                placeholder="巴厘岛, 印度尼西亚"
              />
            </div>
            <Textarea
              label="简短描述"
            ...
            <div className="mt-10 border-t pt-6 space-y-4">
              <div>
                ...
                <p className="text-xs text-gray-500 mt-2">
                  {form.status === "draft"
                    ? "Draft will not be visible to users."
                    : "Package will be live and visible to all users."}
                </p>
              </div>
              <div className="flex items-center">
                ...
                <label
                  htmlFor="featured"
                  className="ml-2 block text-sm text-gray-700 font-medium flex items-center gap-1"
                >
                  <Star className="h-4 w-4 text-yellow-500" /> Mark as Featured
                </label>
              </div>
              <div className="flex justify-between gap-3 pt-2">
                ...
                <div className="flex gap-2">
                  ...
                  <Button
                    onClick={() => {
                      if (form.status === "published" && !mandarinRequiredOk) {
                        addToast("Complete Mandarin required fields", "error");
                        return;
                      }
                      onSubmit();
                    }}
                    isLoading={saving}
                  >
                    {form.status === "published" ? "✓ Publish Package" : "Save"}
                  </Button>
                </div>
              </div>
